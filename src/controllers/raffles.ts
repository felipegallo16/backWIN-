...
export const createNewRaffle = (req: Request, res: Response): void => {
  ...
  const raffle = {
    ...
    creado_por: req.body.admin_id, // ID del administrador que crea el sorteo
...
export const updateRaffleConfig = (req: Request, res: Response): void => {
  ...
  // Verificar si el sorteo está en estado BORRADOR
  if (raffle.configuracion.estado !== 'BORRADOR') {
    // Si está ACTIVO, verificar la ventana de tiempo de 30 minutos
    if (raffle.configuracion.estado === 'ACTIVO') {
      ...
      const ventanaModificacion = 30 * 60 * 1000; // 30 minutos en milisegundos
  ...
  // Validar que no se puedan modificar ciertos campos después de activar
  if (raffle.configuracion.estado === 'ACTIVO') {
...
export const participate = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { raffleId, numero_elegido, proof, action, cantidad_numeros = 1 } = req.body;
  ...
  try {
    ...
    addParticipacion({
      ...
      numero_asignado: numeros_asignados[0], // Por ahora solo guardamos el primer número
      fecha: new Date(),
